...
def generar_curva_consumo(suministro):
    ...
    plt.title(f'Curva de Consumo - Suministro {suministro}')
    plt.legend()
    plt.grid(True)
...
def get_response(msg):
    ...
    if prob.item() > 0.75:
        if tag in last_response and last_response[tag]:
            return f"Ya te proporcione una respuesta sobre {tag}. ¿Necesitas mas ayuda?"
        ...
        for intent in intents["intents"]:
            if tag == intent["tag"]:
                ...
                if tag == "saludos":
                    ...
                    mensaje = f"Corte reportado en Ubicacion del usuario el {fecha_hora}. Se agendara una visita."
...
print(f"{bot_name}. Escribe 'salir' para terminar.")
while True:
    sentence = input("Tú: ")
    if sentence == "salir":
        break
